...
namespace FoxEngine::Graphics
{
	...
	public:
		...
		void SetPosition(const FoxMath::Vector3& position);
		void SetDirection(const FoxMath::Vector3& direction);
		void SetLookAt(const FoxMath::Vector3& target);
		...
		const FoxMath::Vector3& GetPosition() const;
		const FoxMath::Vector3& GetDirection() const;

		FoxMath::Matrix4 GetViewMatrix() const;
		FoxMath::Matrix4 GetProjectionMatrix() const;
	...
	private:
		...
		FoxMath::Vector3 mDirection = FoxMath::Vector3::ZAxis;
		
		// 0 aspect ratio = use back buffer dimension
		float mFov = 60.0f * FoxMath::Constants::DegToRad;
