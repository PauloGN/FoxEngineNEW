...
void FoxEngine::Graphics::VertexShader::Initialize(const std::filesystem::path& filePath, uint32_t format)
{
	//==========================================================
	...
	ASSERT(SUCCEEDED(hr), "Failed to create inputlayout");

	SafeRelease(shaderBlob);
	SafeRelease(errorBlob);
}

void FoxEngine::Graphics::VertexShader::Terminate()
{
...
void FoxEngine::Graphics::VertexShader::Bind()
{
	auto contex = GraphicsSystem::Get()->GetContext();

	contex->VSSetShader(mVertexShader, nullptr, 0);
	contex->IASetInputLayout(mInputLayout);
}
