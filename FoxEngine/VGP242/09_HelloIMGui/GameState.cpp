...
void GameState::Render()
{
	...
	case Shape::S_Sphere:
	{
		SimpleDraw::AddSphere(30, 30, mSphereRadius, Vector3::Zero, mShpereColor);
	}
	break;
...
{
	//With out the flag auto resize we can manually edit the window size 
	ImGui::Begin("Debug UI",nullptr, ImGuiWindowFlags_AlwaysAutoResize);

	int currentItem = (int)eShape;
	...
	//ImGui::ListBox("ShapeTypes##", &currentItem, shapeTypeStr, (int)Shape::S_MAX);
	bool hasChange =  ImGui::Combo("Shape Types##", &currentItem, shapeTypeStr, (int)Shape::S_MAX);
	if(hasChange)
	{
		eShape = (Shape)currentItem;
	}

	//Controller changes according to the shape being rendered
	switch (eShape)
	{
	case Shape::S_Transform: break;
	case Shape::S_Sphere:
		
		ImGui::DragFloat("Sphere Radius##", &mSphereRadius, .2f, .1, 4 );
		ImGui::ColorEdit4("Sphere Color##", &mShpereColor.r);
		break;
	case Shape::S_AABB: break;
	case Shape::S_AABBFilled: break;
	case Shape::S_Custom: break;
	case Shape::S_MAX: break;
	default: ;
	}
